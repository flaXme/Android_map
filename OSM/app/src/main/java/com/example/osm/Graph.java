...
public class Graph {
    ...
    private final int lengthOfEdgeElement = 5;//Edge Element consist of start nodeID, endNodeID , metric vector and two id of subedges of shortcut
    ...
    private void readGraphFile(String path) {
        try {
            ...
            oldNrOfOutgoingEdeges = new int[nodeNr];
            ...
            for (int i = 0; i < nodeNr; i++) {
                if (oldNrOfOutgoingEdeges[i] != nrOfOutgoingEdges[i]) {
                    nodeInCornerCase[i] = 1;
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
